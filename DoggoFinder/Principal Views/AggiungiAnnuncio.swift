...
struct AggiungiAnnuncio: View {
    ...
    var body: some View {
        NavigationView{
                ...
                .onAppear(){
                    ...
                    let nome = dict["nome"] ?? ""
                    let cognome = dict["cognome"] ?? ""
                    let email = dict["email"] ?? ""
                    let telefono = dict["telefono"] ?? ""
                    
                    if !nome.isEmpty && !cognome.isEmpty{
                        self.proprietario = "\(nome) \(cognome)"
                    } else if !nome.isEmpty {
                        self.proprietario = "\(nome)"
                    } else if !cognome.isEmpty {
                        self.proprietario = "\(cognome)"
                    }
                    self.email = email
                ...
                .alert(isPresented: $showConnectionAlert){
                    Alert(
                        ...
                        message: Text("Salvando, il tuo annuncio sarà visibile ad altri utenti appena tornerai online."),
                ...
                .navigationBarTitle("Nuovo annuncio",displayMode: .inline)
    ...
    private func validazioneForm() {
        ...
        if razzaVuoto == nil && cittàVuoto == nil && provinciaVuoto == nil && telefonoVuoto == nil && emailVuoto == nil && emailNonValida == nil && numeroDiTelefonoNonValido == nil {
            ...
            let newAnnuncio = Annuncio(nome: nome.isEmpty ? nil : nome, immagine: immagine?.base64, razza: razza, proprietario: proprietario, città: città, provincia: provincia, telefono: telefono, email: email, idProprietario: idUtente, note: note.isEmpty ? nil : note, latitudine: latitudine, longitudine: longitudine)
